	...
	public static void main (String[] args) {
		...
		if (SharedLibraryLoader.isWindows) {
			if (System.getProperties().getProperty("os.name").equals("Windows XP")) {
				basePath = "Application Data/.thiefpixel/Thief Pixel Dungeon/";
			} else {
				basePath = "AppData/Roaming/.thiefpixel/Thief Pixel Dungeon/";
			}
			...
			basePath = XDGHome + ".thiefpixel/thief-pixel-dungeon/";

			//copy over files from old linux save DIR, pre-1.2.0
			FileHandle oldBase = new Lwjgl3FileHandle(".thiefpixel/thief-pixel-dungeon/", Files.FileType.External);
			FileHandle newBase = new Lwjgl3FileHandle(XDGHome + ".thiefpixel/thief-pixel-dungeon/", Files.FileType.External);
			if (oldBase.exists()){
				if (!newBase.exists()) {
