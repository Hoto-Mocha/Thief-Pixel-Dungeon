...
public class Thief_3 extends Mob {
	...
	
	{
		...
		lootChance = 0.3f; //initially, see lootChance()
	...
	protected boolean steal( Hero hero ) {
		...
		if (toSteal != null && !toSteal.unique && toSteal.level() < 1 ) {
			...
			GLog.w( Messages.get(Thief_3.class, "stole", toSteal.name()) );
		...
		protected void nowhereToRun() {
			if (buff( Terror.class ) == null
				...
				if (enemySeen) {
					...
					int newPos;
					do {
						newPos = Dungeon.level.randomRespawnCell( Thief_3.this );
						if (count-- <= 0) {
							break;
						}
					...
					if (item != null) GLog.n( Messages.get(Thief_3.class, "escapes", item.name()));
					item = null;
					state = WANDERING;
				} else {
					state = WANDERING;
