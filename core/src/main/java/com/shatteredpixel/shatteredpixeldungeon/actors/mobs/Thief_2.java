...
public class Thief_2 extends Mob {
	...
	protected boolean steal( Hero hero ) {
		...
		if (toSteal != null && !toSteal.unique && toSteal.level() < 1 ) {
			...
			GLog.w( Messages.get(Thief_2.class, "stole", toSteal.name()) );
		...
		protected void nowhereToRun() {
			if (buff( Terror.class ) == null
				...
				if (enemySeen) {
					...
					int newPos;
					do {
						newPos = Dungeon.level.randomRespawnCell( Thief_2.this );
						if (count-- <= 0) {
							break;
						}
					...
					if (item != null) GLog.n( Messages.get(Thief_2.class, "escapes", item.name()));
					item = null;
					state = WANDERING;
				} else {
					state = WANDERING;
